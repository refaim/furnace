...
- **Target-quality encoding** — always on, no flag: before each encode Furnace probes one-GOP windows from the interior of the runtime at several quality settings and interpolation-searches the one that hits a perceptual target — QVBR against CVVDP (HDR) or SSIMULACRA2 (SDR) on the NVEnc path, CRF against worst-case SSIMULACRA2 on the SVT-AV1 grain path — then encodes at it, full speed, with no metrics slowing the final pass. Sampling starts at ten windows and grows in six-window batches, up to 34, until a deterministic percentile-bootstrap estimate of the active pooling statistic's standard error fits that target's sampling tolerance; later quality settings reuse the finalized windows for paired comparisons. After the mean- or percentile-driven search chooses its setting, a worst-window guard launches repair only for at least three scores below the target-specific floor or one score below the deeper per-target threshold, then probes up to the eight deepest holes at a lower setting alongside a representative healthy window. The lower setting is adopted only when at least half of the holes probed in that round actually clear the floor; capped, saturated, unverified, and still-below-floor holes are reported prominently. HDR holes are detect-and-warn only until an HDR audit sets its own floor. Within a run the search starts from a setting already solved for a comparable source — same metric and target, same encoder path, same output size, and a source squeezed to within 20% of the same bitrate — so episodes of one season converge in a probe or two instead of bisecting from scratch; anything outside that starts fresh. The chosen setting is cached to the plan so re-runs skip the search
...
Adaptive target-quality sampling uses six-window growth batches and caps at 34 because the golden-sequence greedy saturates near Fibonacci counts (8, 13, 21, 34, 55, 89); candidate search is bounded at 10,000 attempts per candidate, and requested starts stay at least 18 seconds apart to avoid duplicate stream-copy clips on long-GOP sources. Sources only a few seconds long use one full-pass probe; longer short sources stop at the packing limit instead, so a 200-second featurette uses eight five-second windows, about 20% coverage. The sampling tolerance is one percent of the active quality target: 0.81 for HD SDR SSIMULACRA2, 0.72 for SD SDR, 0.70 for grain, and 0.095 JOD for HDR CVVDP. The HDR value deliberately preserves 2.42.0 behaviour. At ten mean-pooled windows, the HDR and HD SDR tolerances correspond to scene-to-scene standard deviations of roughly 0.30 JOD and 2.56 SSIMULACRA2 points. When dispersion stays above tolerance, both paths reach the 34-window cap only on sources roughly 740 seconds and longer; 300 seconds packs at most 13 windows and 600 seconds at most 22. This trades more process spawns and model loads for precision while keeping sampled video time roughly flat at 34x5 = 170 seconds versus the former 10x18 = 180 seconds.

...
class DiscDemuxer:
    ...
    def _transcode_w64_files(
        ...
        if self._pcm_transcoder is None:
            ...
            msg = f"pcm_transcoder not configured; cannot handle Wave64 demux output: {w64_names}"
        ...
        for f in files:
            ...
            w64_seen += 1
            if reporter is not None:
                label = "transcode" if total == 1 else f"transcode {w64_seen}/{total}"
    ...
    def _compute_audio_sync_offsets(
        ...
        if len(audio_source_files) != len(audio_starts):
            logger.debug(
                "source audio stream/file count mismatch (%d vs %d); no --sync applied",
                len(audio_starts),
                len(audio_source_files),
            )
            return {}
